...
> 建立：2026-04-19 · 维护：Alice、Claude Opus 4.7、Claude Opus 4.6 与 GPT-5.4
...
Claude Opus 4.7 的后续版本。**继续维护者与联合作者**：

- 设计并实现 `constellatio` 阅读 skill（接受史分析 + 屏幕属性识别 + 暗夜星图 viewer）
- 重构 constellatio 架构：散文与图表解耦为同级产物，消除图表词汇对认知方法的污染
- 运行时诚实化：清理未实现的 runtime 声明，版本号统一推进至 v0.2.0
- 合并三份参考文件为 `viewer-spec.md`，建立词汇防火墙

在署名意义上，Claude Opus 4.6 与 Claude Opus 4.7 同为本项目的联合作者。
...
| Constellatio skill | Claude Opus 4.6 | 接受史分析 + 屏幕属性识别 + viewer-spec 合并 |
| 运行时诚实化 | Claude Opus 4.6 | 清理未实现 runtime 声明、v0.2.0 版本管理 |
...
- **联合作者**：Alice、Claude Opus 4.7、Claude Opus 4.6 与 GPT-5.4
- **代码许可**：[Apache License 2.0](LICENSE)
- **引用材料许可**：[CC BY 4.0](LICENSING.md)

在引用、转载、改编本项目的任何部分时，请同时标注版权持有人（Alice）与联合作者（Alice、Claude Opus 4.7、Claude Opus 4.6 与 GPT-5.4），以及对应许可证的引用链接。
