...
从 `<workspace>.ocr/prep/pages/page_001.png` 采样第一页做**快速视觉判断**（Agent 直接看图，不调脚本）。未跑过 prep-scan 时（无 `prep/pages/`），退化为让用户直接描述或随 MinerU `-l auto`：
...
> `prep/pages/` 由 prep-scan 生成。若用户跳过 prep-scan 直接 OCR，该目录不存在——`make_preview.py` 会降级为纯文本预览（无原图）。

HTML 是**本地文件，不联网**。两种使用模式：

- **Agent 自主模式（默认）**：`previews/ocr-preview.html` 仅作为 diff-review 的视觉参照，Agent 不依赖 human-in-browser 编辑。直接进入下一步 proofread。
...
跑完 OCR 后重写工作区 README.md（目录地图）：

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/workspace_readme.py" --workspace "$OUT"
```
...
- 对照预览：   $OUT/previews/ocr-preview.html（视觉参照，不需手工编辑）
...
```
OCR 完成。人工协作流程：
1. 打开 $OUT/previews/ocr-preview.html 核验 / 手改
...
```bash
open "$OUT/previews/ocr-preview.html"
```
