---
name: visual-preview
description: 使用场景：prep-scan 跑完后，用户说"看看清理效果""对比一下""去水印成功没""擦掉了什么""这页裁了多少""效果怎么样""让我看看结果""预览处理""visual preview"等。这个 skill 读 `<name>.prep/pages/`（原始）和 `<name>.prep/cleaned_pages/`（清理后），生成一份可直接在浏览器打开的 HTML：每页三态切换（原图 / 清理后 / 差异热图 —— 擦掉的水印馆藏章用半透明红色高亮），顶部汇总（总页数、平均清理比例、裁边总量），让用户一眼看出 prep-scan 到底做了什么、有没有误伤正文。主动触发：prep-scan 结束后用户一说"看看""对比""瞧瞧"就走，不必等用户说完整命令名。
...
# 用户可能传：
#   (a) PDF 路径  ~/Downloads/论文.pdf     → 推断 论文.prep/
#   (b) prep 目录 ~/Downloads/论文.prep/   → 直接用
if [ -d "$INPUT" ]; then
    PREP="$INPUT"
    ...
    echo "找不到 prep 目录或 PDF：$INPUT"; exit 1
fi
...
- 内联 CSS，无外链
- 图片用**相对路径**引用（`pages/page_001.png` 等），让 HTML 跟 prep 目录一起挪动
- 顶部汇总卡片
- 每页一个 section：
